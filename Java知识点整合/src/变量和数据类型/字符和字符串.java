package 变量和数据类型;

public class 字符和字符串{
    public static void main(String[] args) {
    ChartoInt();
    ZiFuChuan();
    ZhuanYi();
    CannotChange();
    CannotChange1();
    Null();
    }

    /**
     *字符类型char是基本数据类型，它是character的缩写。一个char保存一个Unicode字符
     * Java在内存中总是使用Unicode表示字符，
     * 所以，一个英文字符和一个中文字符都用一个char类型表示，它们都占用两个字节。
     * 要显示一个字符的Unicode编码，只需将char类型直接赋值给int类型即可
     */
    public static void ChartoInt(){
        int n1 = 'A'; // 字母“A”的Unicodde编码是65
        int n2 = '中'; // 汉字“中”的Unicode编码是20013
        //还可以直接用转义字符Unicode编码来表示一个字符：
        // 注意是十六进制:
        char c3 = '\u0041'; // 'A'，因为十六进制0041 = 十进制65
        char c4 = '\u4e2d'; // '中'，因为十六进制4e2d = 十进制20013
        System.out.println("n1 = "+ n1+ "n2  ="+ n2 + "c3 = " + c3 +"c4 = "+c4 );
    }

    /**
     * 字符串类型
     * 和char类型不同，字符串类型String是引用类型，
     * 我们用双引号"..."表示字符串。一个字符串可以存储0个到任意个字符
     */
    public static void ZiFuChuan(){
        String s = ""; // 空字符串，包含0个字符
        String s1 = "A"; // 包含一个字符
        String s2 = "ABC"; // 包含3个字符
        String s3 = "中文 ABC"; // 包含6个字符，其中有一个空格
        String s4 = "abc\"xyz";  // 包含7个字符: a, b, c, ", x, y, z
        String s5 = "abc\\xyz"; // 包含7个字符: a, b, c, \, x, y, z
        System.out.println();
    }

    /**
     * 常见转义字符
     * \" 表示字符"
     * \' 表示字符'
     * \\ 表示字符\
     * \n 表示换行符
     * \r 表示回车符
     * \t 表示Tab
     */

    public static void ZhuanYi(){
        String s1 = "Hello";
        String s2 = "world";
        String s = s1 + " " + s2 + "!";
        System.out.println(s);
        //如果用+连接字符串和其他数据类型，会将其他数据类型先自动转型为字符串，再连接
        int age = 25;
        String z = "age is " + age;
        System.out.println(z);
    }

    /**
     * 多行字符串
     * 要表示多行字符串，使用+号连接会非常不方便
     * Ex：String s = "first line \n"
     *          + "second line \n"
     *          + "end";
     * 从Java 13开始，字符串可以用"""..."""表示多行字符串（Text Blocks）
     * 目前版本1。8，无法运行
     */
    /*public static void DuoHangZiFu(){
        String s = """
                   SELECT * FROM
                     users
                   WHERE id > 100
                   ORDER BY name DESC
                   """;
        System.out.println(s);
    }*/

    /**
     * 不可变特性
     * Java的字符串除了是一个引用类型外，还有个重要特点，就是字符串不可变。考察以下代码
     *
     */
    public static void CannotChange(){
        String s = "hello";
        System.out.println(s); // 显示 hello
        s = "world";
        System.out.println(s); // 显示 world
    }

    /*
     * 观察执行结果，难道字符串s变了吗？其实变的不是字符串，而是变量s的“指向”。
     *
     * 执行String s = "hello";时，JVM虚拟机先创建字符串"hello"，然后，把字符串变量s指向它：
     *
     *       s
     *       │
     *       ▼
     * ┌───┬───────────┬───┐
     * │   │  "hello"  │   │
     * └───┴───────────┴───┘
     * 紧接着，执行s = "world";时，JVM虚拟机先创建字符串"world"，然后，把字符串变量s指向它：
     *
     *       s ──────────────┐
     *                       │
     *                       ▼
     * ┌───┬───────────┬───┬───────────┬───┐
     * │   │  "hello"  │   │  "world"  │   │
     * └───┴───────────┴───┴───────────┴───┘
     * 原来的字符串"hello"还在，只是我们无法通过变量s访问它而已。
     * 因此，字符串的不可变是指字符串内容不可变
     */
    public static void CannotChange1(){
        String s = "hello";
        String t = s;
        s = "world";
        System.out.println(t); // t是"hello"还是"world"?
    }
    /**
     * 空值null
     * 引用类型的变量可以指向一个空值null，它表示不存在，即该变量不指向任何对象。
     */
    public static void Null (){
        String s1 = null; // s1是null
        String s2; // 没有赋初值值，s2也是null
        String s3 = s1; // s3也是null
        String s4 = ""; // s4指向空字符串，不是null
        System.out.println("All the four S can print nothing");
    }


}
