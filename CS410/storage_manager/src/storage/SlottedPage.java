 ...
 * Assignment 1
 * Pass the add(), get(), remove(), iterator(), method in SlottedPageTest
 * Pass the put(), get(), remove() method in FileManagerTest
 * imcomplete elements:
 *    compact(); in SlottedPage
 *    Iterator(); in FileManager
 *    Section 3;
 * Spend about 10 days to finish it.
 * 
 * Suggestions:
 * It is my first time to use the Junit, spend a lot of time to learn it
 * Student Name: Shunjie Wan
 */
...
public class SlottedPage implements Iterable<Object> {
   ...
   * @throws OverflowException if this {@code SlottedPage} cannot accommodate the specified object
   */
  public int add(Object o) throws IOException, OverflowException {
    // TODO complete this method (20 points)
    try {
      //call the save method and
      saveLocation(entryCount(), save(o));
      //set the number of the entry to a int
      setEntryCount(entryCount() + 1);
      //the index of the object
      return entryCount() - 1;

    } catch (OverflowException e) {
      //catch the OverflowException exception
  ...
  public Object get(int index) throws IndexOutOfBoundsException, IOException {
    // TODO complete this method (20 points)
    try {
      //if the location = -1, means the object has been removed, then return null
      if (getLocation(index) == -1) {
        return null;
      } else if (index < 0) {
        //throw IndexOutOfBoundsException if the index is not correct
        throw new IndexOutOfBoundsException();
      } else {
        //get the object at this index
        return toObject(data, getLocation(index));
      }
    } catch (Exception e) {
   ...
   *
   * @param index an index within this {@code SlottedPage}
   * @return the object stored previously at the specified location in this {@code SlottedPage}; {@code null} if no
   * such object
   * @throws IndexOutOfBoundsException if an invalid index is used
   * @throws IOException               if an I/O error occurs
  ...
  public Object remove(int index) throws IndexOutOfBoundsException, IOException {
    ...
    //get the previously object at the specified location before remove, save it as preValue
    Object preValue = get(index);
    try {
      //remove the object at this index,  then save the location as -1
      saveLocation(index, -1);
      //return the previously object at this specified location
      return preValue;
    } catch (Exception e) {
  ...
  public Iterator<Object> iterator() {
    // TODO complete this method (10 points)
    try {
      //create a ArrayList to store the object
      ArrayList<Object> list = new ArrayList<>();
      for (int i = 0; i < entryCount(); i++) {
        if (get(i) == null) {
          //if the object is null, then skip this null object
          continue;
        }
        list.add(get(i));
      }
   ...
   *
   * @param count the number of entries in this {@code SlottedPage}
   */
   ...
   *
   * @param index an index that specifies an object.
   ...
   *
   * @param index    an index that specifies an object.
   * @param location a location within this {@code SlottedPage}
   */
   ...
   *
   * @param location a location in the byte array of this {@code SlottedPage}
